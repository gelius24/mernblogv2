...
export default function UpdatePost() {
  ...
  // fetch the post title, category, image, content, _id and set it in formData
  useEffect(() => {
    const fetchPost = async () => {
      try {
        ...
        if (res.ok) {
          ...
          setFormData({title, category, image, content, _id})
        } else {
  ...
  const handleUploadImage = async () => {
    try {
      if (!file) return setImageUploadError('No image selected')
  ...
  // send the post to mongodb with a http POST req with formData in the body.
  ...
  return (
    <div className="p-3 max-w-3xl mx-auto min-h-screen">
      ...
      <form className="flex flex-col gap-4" onSubmit={handleSubmit}>
        <div className="flex flex-col gap-4 sm:flx-row justify-between">
        ...
        {formData.image && <img src={formData.image} className='w-full h-72 object-cover' alt='upload'/>}
